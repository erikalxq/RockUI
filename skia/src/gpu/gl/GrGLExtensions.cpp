...
bool GrGLExtensions::init(GrGLStandard standard,
    ...
    GrGLVersion version = GrGLGetVersionFromString(/*(const char*) verString*/"4.3.0 - Build 20.19.15.4352");
    ...
    if (indexed) {
        ...
        for (int i = 0; i < extensionCnt; ++i) {
			...
			SkTArray<SkString>* pArrayString = fStrings.get();
			if (pArrayString == NULL)
			{
				continue;
			}
