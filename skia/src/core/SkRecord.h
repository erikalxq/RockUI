...
public:
    ...
    R mutate(unsigned i, F& f) {
        SkASSERT(i < this->count());
        return fRecords[int(i)].mutate<R>(fTypes[int(i)], f);
    }
    ...
    T* append() {
        ...
        fTypes[int(fCount)] = T::kType;
        return fRecords[int(fCount++)].set(this->allocCommand<T>());
    }
    ...
    T* replace(unsigned i) {
        ...
        fTypes[(int)i] = T::kType;
        return fRecords[(int)i].set(this->allocCommand<T>());
    }
