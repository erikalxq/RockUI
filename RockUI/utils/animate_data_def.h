...
namespace ease_tween
{
	...
	interface IKEaseTweenSimpleAnimateData
	{
		virtual void Clear() = 0;
		virtual float AddTime(float fAddTime) = 0;
		virtual void SetValue(float fTotalTime, float fStartValue, float fChangeValue) = 0;
		virtual float Caculate() const = 0;
		virtual float IsStop() const = 0;
	};

template<class TEaseTween>
class KEaseTweenSimpleAnimateData : public IKEaseTweenSimpleAnimateData
{
public:
    ...
    virtual float AddTime(float fAddTime)
    {
